...
% To reproduce Table 1 for the 1D UCI datasets.
...
% Computation should take 25s with the given parameters for the dataset
% "ftcollinssnow". As it depends on the total number of points, it varies a
% lot between datasets. The total gridsearch could last days.

NAMES_1D = ["engel","GAGurine", "geyser","mcycle","ftcollinssnow"];
...
% listFiles=1:length(NAMES_1D); %UNCOMMENT TO CONSIDER ALL THE 1D DATASETS
% savebool=true; %UNCOMMENT TO SAVE THE RESULTS

for nIter=1:maxIter
    tic
for fileNb=listFiles
load(strcat(NAMES_1D(fileNb),".mat"))
varX=var(X); nX=size(X,1);
smIndx=1:nX; Y=y(smIndx);%randperm(size(X,1),50) 
[X,reInd]=sort(X(smIndx,:));Y=Y(reInd);

ntest=100; xgrid=linspace(-0.1+min(X),max(X)+0.1,ntest); 
...
plossCV_hyper=zeros(length(GridsPrct),length(GridboundN));
    ...
    [Xnew,eta] = CompXgapEta1D(X,sigX,h); n=size(Xnew,1);
...
if savebool
    saveFileName=['QR_Table_Results_1D.txt'];

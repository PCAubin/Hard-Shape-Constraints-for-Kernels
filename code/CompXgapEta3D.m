function [Xnew,eta] = CompXgapEta3D(Xorig,sigX,h,nbPts_per_dim)
%This function computes the eta parameters for a uniform grid in 3D as
...
%the original points.
X=Xorig;
m1=min(Xorig(:,1)); m2=min(Xorig(:,2)); m3=min(Xorig(:,3));
if nargin<4
    nbPts_per_dim=10;
end
...
zCoord=m3+deltaDesi(3)*(1/2+(0:nbPts_per_dim-1));
[Xtemp,Ytemp,Ztemp] = meshgrid(xCoord,yCoord,zCoord);
Xgap=[Xtemp(:),Ytemp(:),Ztemp(:)];
...
if any(deltaDesi>sigX)
    disp(["The new points were still too far for sig_X = ", num2str(sigX), ", the property will not be valid on the whole interval."])
